 ...
 *
 * Per frame, the hook:
 *   - Reads each wave's dials once → plain Wave for the whole frame.
  ...
  return useMemo<BeamFn>(() => {
    ...
    return (t, dt) => {
      ...
      for (let si = 0; si < ps.length; si++) {
        ...
        // One tree read per wave per frame. Dial values are constant
        // within a tick (fixed ctx), and stateful sources/glide advance
        // per tick — reading inside the sample loop would just walk the
        // tree tens of thousands of times for identical values.
